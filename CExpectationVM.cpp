...
CExpectationVM::TimeSample::TimeSample() :
	t(),
	v()
{
}

CExpectationVM::TimeSample::TimeSample(long t_, float v_) :
	t(t_),
	v(v_)
{
}
...
int CExpectationVM::importFromArray(double* array,int len)
{
	...
	if (type != TT_EXPECTATION_VM) std::cerr << "Error loading the model, type mismatch." << std::endl;
