...
double CExpectation::time_to_phase(uint32_t time) {
...
{
	std::cout << "Expectation Maximisation" << std::endl;
...
CExpectation::Cluster::Cluster(double kappa_, double mu_, double weight_) :
	kappa(kappa_),
	mu(mu_),
	weight()
{
...
void CExpectation::Cluster::print() {
	std::cout << "(" << kappa << ", " << mu << ", " << weight << ")" << std::endl;
}


void CExpectation::Cluster::save(FILE* file, bool lossy) {
	fwrite(&kappa, sizeof(double), 1, file);
	fwrite(&mu, sizeof(double), 1, file);
	fwrite(&weight, sizeof(double), 1, file);
}

void CExpectation::Cluster::load(FILE* file) {
	fread(&kappa, sizeof(double), 1, file);
	fread(&mu, sizeof(double), 1, file);
	fread(&weight, sizeof(double), 1, file);
}

void CExpectation::Cluster::importFromArray(double* array, int len, int& pos) {
	kappa = array[pos++];
	mu = array[pos++];
	weight = array[pos++];
}

void CExpectation::Cluster::exportToArray(double* array, int maxLen, int& pos) {
	array[pos++] = kappa;
	array[pos++] = mu;
	array[pos++] = weight;
}

double CExpectation::Cluster::density_at(double phase) const {
	return exp(kappa*cos(phase - mu)) / (2*M_PI*gsl_sf_bessel_I0(kappa));
}

void CExpectation::Cluster::estimate_from_mean(double re, double im) {
	if (re == 0 && im == 0) {
		mu = 0;
		kappa = 0;
		return;
...
double CExpectation::Cluster::mean_f (double x, void* params) {
	double* y = (double*) params;
	return gsl_sf_bessel_I1(x) / gsl_sf_bessel_I0(x) - *y;
}

double CExpectation::Cluster::mean_df (double x, void* params) {
	double i0 = gsl_sf_bessel_I0(x);
	double i1 = gsl_sf_bessel_I1(x);
	return (i0*(i0 + gsl_sf_bessel_In(2, x)) + 2*i1*i1) / (i0*i0);
}

void CExpectation::Cluster::mean_fdf (double x, void* params, double* y, double* dy) {
